...
class AnimalDetector {
    ...
    drawDetections(canvas, image, detections) {
        ...
        // Draw original image without any modifications
        ctx.drawImage(image, 0, 0);

        // Draw detections with larger, more prominent labels
        detections.forEach(detection => {
            ...
            // Draw bounding box with much thicker border
            ctx.strokeStyle = colorStr;
            ctx.lineWidth = 6; // Increased from 4 to 6 for more prominent border
            ctx.strokeRect(x1, y1, boxWidth, boxHeight);

            // Prepare label text with class name and confidence
            const confidencePercent = (detection.confidence * 100).toFixed(1);
            const label = `${detection.className.en} ${confidencePercent}%`;
            
            // Configure much larger font for label
            ctx.font = 'bold 28px Arial'; // Increased from 20px to 28px
            const textMetrics = ctx.measureText(label);
            const textWidth = textMetrics.width;
            const textHeight = 32; // Increased height for larger font
            const padding = 12; // Increased padding for better spacing

            // Calculate label position (above the box, or inside if no space)
            let labelX = x1;
            let labelY = y1 - textHeight - padding * 2 - 6;

            // If label would go above image bounds, place it inside the box at top
            if (labelY < 0) {
                labelY = y1 + 6;
            }

            // Draw label background with solid color and rounded corners effect
            ctx.fillStyle = colorStr;
            ctx.fillRect(labelX, labelY, textWidth + padding * 2, textHeight + padding * 2);

            // Draw label text with white color for maximum contrast
            ctx.fillStyle = 'white';
            ctx.textBaseline = 'top';
            ctx.textAlign = 'left';
            ctx.fillText(label, labelX + padding, labelY + padding);

            // Draw larger corner indicators (Roboflow style)
            const cornerSize = 24; // Increased from 16 to 24
            const cornerThickness = 6; // Increased from 4 to 6
            ctx.fillStyle = colorStr;
            
            // Top-left corner
            ctx.fillRect(x1 - 3, y1 - 3, cornerSize, cornerThickness);
            ctx.fillRect(x1 - 3, y1 - 3, cornerThickness, cornerSize);
            
            // Top-right corner
            ctx.fillRect(x2 - cornerSize + 3, y1 - 3, cornerSize, cornerThickness);
            ctx.fillRect(x2 - 3, y1 - 3, cornerThickness, cornerSize);
            
            // Bottom-left corner
            ctx.fillRect(x1 - 3, y2 - cornerSize + 3, cornerThickness, cornerSize);
            ctx.fillRect(x1 - 3, y2 - cornerThickness + 3, cornerSize, cornerThickness);
            
            // Bottom-right corner
            ctx.fillRect(x2 - cornerSize + 3, y2 - cornerThickness + 3, cornerSize, cornerThickness);
            ctx.fillRect(x2 - 3, y2 - cornerSize + 3, cornerThickness, cornerSize);

            // Add larger confidence badge at bottom of box for bigger detections
            if (boxWidth > 120 && boxHeight > 120) {
                const badgeText = `${confidencePercent}%`;
                ctx.font = 'bold 18px Arial'; // Increased from 14px to 18px
                const badgeMetrics = ctx.measureText(badgeText);
                const badgeWidth = badgeMetrics.width + 16; // More padding
                const badgeHeight = 26; // Increased height
                const badgeX = x1 + (boxWidth - badgeWidth) / 2;
                const badgeY = y2 - badgeHeight - 10;
